...
export default function DepositForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const depositCurrency = hasWallets && selectedWallet ? selectedWallet.asset : currency;
    const depositMethod   = hasWallets && selectedWallet ? selectedWallet.label : method;
    ...
    <div className="max-w-xl mx-auto space-y-5">
      ...
      <div className="glass-card rounded-xl p-4 border border-yellow-500/20 bg-yellow-500/5 flex items-start gap-3">
        ...
        <div className="text-xs text-slate-400 leading-relaxed">
          ...
          {hasWallets
            ? "Send only the selected asset and network. Double-check the wallet address before sending."
            : "All deposits are reviewed by our finance team before funds are credited."}
        </div>
      </div>
      ...
      <Card className="glass-card border-0 rounded-xl p-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {hasWallets ? (
            <>
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400 uppercase tracking-widest">Select Asset / Network <span className="text-red-400">*</span></Label>
                <Select value={walletId} onValueChange={(v) => v && setWalletId(v)}>
                  <SelectTrigger className="bg-white/5 border-white/10 text-white h-11"><SelectValue placeholder="Choose an asset to deposit" /></SelectTrigger>
                  <SelectContent className="bg-[#0d1e3a] border-sky-500/20 text-white">
                    {wallets.map((w) => (<SelectItem key={w.id} value={w.id} className="hover:bg-sky-500/10 focus:bg-sky-500/10">{w.label}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              {selectedWallet && (
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2 p-3 bg-white/3 rounded-lg border border-white/5">
                    <code className="flex-1 text-xs text-sky-300 font-mono break-all select-all">{selectedWallet.address}</code>
            ...
            <>
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400 uppercase tracking-widest">Currency <span className="text-red-400">*</span></Label>
                <Select value={currency} onValueChange={(v) => v && setCurrency(v)}>
                  <SelectTrigger className="bg-white/5 border-white/10 text-white h-11"><SelectValue /></SelectTrigger>
                  <SelectContent className="bg-[#0d1e3a] border-sky-500/20 text-white">
                    {CURRENCIES.map((c) => (<SelectItem key={c} value={c} className="hover:bg-sky-500/10 focus:bg-sky-500/10">{c}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400 uppercase tracking-widest">Deposit Method <span className="text-red-400">*</span></Label>
                <Select value={method} onValueChange={(v) => v && setMethod(v)}>
                  <SelectTrigger className="bg-white/5 border-white/10 text-white h-11"><SelectValue /></SelectTrigger>
                  <SelectContent className="bg-[#0d1e3a] border-sky-500/20 text-white">
                    {METHODS.map((m) => (<SelectItem key={m} value={m} className="hover:bg-sky-500/10 focus:bg-sky-500/10">{m}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
            </>
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-slate-400 uppercase tracking-widest">Amount <span className="text-red-400">*</span></Label>
            <Input type="number" step="0.01" min={minDeposit} max={maxDeposit ?? undefined} placeholder={`Min ${minDeposit}`}
              value={amount} onChange={(e) => setAmount(e.target.value)}
              className="bg-white/5 border-white/10 text-white placeholder:text-slate-600 h-11" required />
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="Optional — paste your tx hash or transfer reference" value={txHash}
            ...
            {proofFile ? (
              <div className="flex items-center gap-3 rounded-lg px-4 py-3 bg-sky-500/[0.07] border border-sky-500/30">
                ...
                <button type="button" onClick={() => { setProofFile(null); if (fileRef.current) fileRef.current.value = ""; }}
                  className="text-slate-400 hover:text-white transition-colors flex-shrink-0"><X size={16} /></button>
              </div>
            ) : (
          ...
          <Button type="submit" disabled={loading || isRestricted}
            ...
            {loading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Submitting...</>)
              : isRestricted ? "Verification Required"
